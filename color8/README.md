...
| | |
|---|---|
| ![heat_color()](../gallery/heat_sweep.png) `cargo run --example heat_sweep` — [`heat_color`] over the full `u8` range. | ![hsv2rgb_rainbow()](../gallery/rainbow_sweep.png) `cargo run --example rainbow_sweep` — [`hsv2rgb_rainbow`] across all 256 hues. |
| ![fill_rainbow_circular()](../gallery/rainbow_wheel.png) `cargo run --example rainbow_wheel` — [`fill_rainbow_circular`], one turn spread over a ring. | ![fill_gradient_rgb variants](../gallery/gradient_fills.png) `cargo run --example gradient_fills` — 2/3/4-stop [`fill_gradient_rgb`]/[`fill_gradient_rgb3`]/[`fill_gradient_rgb4`]. |
| ![color_from_palette16()](../gallery/palette_blend.png) `cargo run --example palette_blend` — [`color_from_palette16`], 16 colors interpolated smoothly. | ![blend()](../gallery/crossfade.png) `cargo run --example crossfade` — [`blend`] at six steps between two colors. |

[`heat_color`]: src/heat.rs
[`hsv2rgb_rainbow`]: src/hsv.rs
[`fill_rainbow_circular`]: src/fill.rs
[`fill_gradient_rgb`]: src/fill.rs
[`fill_gradient_rgb3`]: src/fill.rs
[`fill_gradient_rgb4`]: src/fill.rs
[`color_from_palette16`]: src/palette.rs
[`blend`]: src/blend.rs
